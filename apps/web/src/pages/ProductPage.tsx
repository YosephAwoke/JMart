...
export function ProductPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!mounted) return;
      const resolved = detail ?? mockCatalog.find((item) => item.slug === slug) ?? null;
      if (resolved) setProduct((current) => current ?? resolved);
      else setProduct((current) => current ?? initialProduct);
        ...
        <motion.div layout className="space-y-4 rounded-[2rem] border border-border bg-surface p-4 shadow-premium">
          ...
          >
            <motion.img
              ...
              initial={{ scale: 1.05, opacity: 0.5, y: 6 }}
            ...
            <div className="absolute right-4 top-4 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-slate-900 backdrop-blur">
              Zoom + swipe gallery
            </div>
